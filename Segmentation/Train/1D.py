...
exponents_val = np.loadtxt('../Validation/positions_1D.txt')

# Run model
model = segmentation_model_1d()
model.compile(optimizer=Adam(learning_rate=0.0001), loss='sparse_categorical_crossentropy', metrics=['accuracy'])
model.summary()
history = model.fit(track_generator_segmentation(dimensions=1, n=32, track_length=200), steps_per_epoch=200, epochs=100,
                    callbacks=[
